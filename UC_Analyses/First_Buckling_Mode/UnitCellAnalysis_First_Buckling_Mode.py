# # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # #
#                                                                                                 #
# UnitCellAnalysis_First_Buckling_Mode.py                                                         #
# Parametric unit cell model using first eigenmode in Load Case 1 as initial distortion           #
# Units: Kilograms-Meters-Seconds                                                                 #
# Initiated by Dennis Hagnas on Tue Jul 22 2025                                                   #
#                                                                                                 #
# # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # #
...
mdb.models['Model-1-Neg'].StaticRiksStep(name = 'Step-1', previous = 'Initial', maxNumInc = 10000,
    initialArcInc = 0.0001, minArcInc = 1e-10, maxArcInc = 0.005, nlgeom = ON, maxLPF = 1)
# -------------------------------------------------------------------------------------------------
# Create u3 = 0 BC
...
mdb.Model(name = 'Model-1-Eigen', objectToCopy = mdb.models['Model-1-Neg'])
mdb.models['Model-1-Eigen'].BuckleStep(name='Step-1', previous='Initial', 
    maintainAttributes = True, numEigen = 5, vectors = 10)
mdb.Job(name = 'Job-1-Eigen', model = 'Model-1-Eigen', description = '', type = ANALYSIS,  
    explicitPrecision = SINGLE, nodalOutputPrecision = SINGLE, echoPrint = OFF, modelPrint = OFF, 
    contactPrint = OFF, historyPrint = OFF, numCpus = 1, numGPUs = 0).submit()
# -------------------------------------------------------------------------------------------------
...
mdb.jobs['Job-1-Eigen'].waitForCompletion()
imp = 0.003
def submitJob(modelNo):
    ...
    mdb.Job(name = job2, model = model2, description = '', type = ANALYSIS, 
        explicitPrecision = SINGLE, nodalOutputPrecision = SINGLE, echoPrint = OFF, 
        modelPrint = OFF, contactPrint = OFF, historyPrint = OFF, numCpus = 1, numGPUs = 0)
    mdb.jobs[job1].writeInput()
    mdb.jobs[job2].writeInput()
    with open(job1+'.inp', 'r') as file:
        lines = file.readlines()
    insertIndex = None
    for i, line in enumerate(lines):
        if line.strip().startswith('*End Assembly'):
            insertIndex = i + 1
            break
    if insertIndex:
        imperfectionBlock = [
            '**',
            '*IMPERFECTION, FILE=Job-1-Eigen, STEP=1',
            '1, '+str(imp)+','
            '**'
        ]
        lines[insertIndex:insertIndex] = [line + '\n' for line in imperfectionBlock]
        with open(job1+'.inp', 'w') as file:
            file.writelines(lines)
    with open(job2+'.inp', 'r') as file:
        lines = file.readlines()
    insertIndex = None
    for i, line in enumerate(lines):
        if line.strip().startswith('*End Assembly'):
    ...
    if insertIndex:
        ...
        lines[insertIndex:insertIndex] = [line + '\n' for line in imperfectionBlock]
        with open(job2+'.inp', 'w') as file:
            file.writelines(lines)
    mdb.JobFromInputFile(name = job1+'Modified', inputFileName = job1+'.inp')
    mdb.JobFromInputFile(name = job2+'Modified', inputFileName = job2+'.inp')
    mdb.jobs[job1+'Modified'].submit()
    mdb.jobs[job2+'Modified'].submit()
    mdb.jobs[job1+'Modified'].waitForCompletion()
    mdb.jobs[job2+'Modified'].waitForCompletion()
mdb.models['Model-1-Pos'].steps['Step-1'].setValues(initialArcInc = 0.001, maxArcInc = 0.005)
submitJob(1)
# ----------------------------------------------------------------------------------------------- #
...
mdb.models['Model-2-Pos'].steps['Step-1'].setValues(maxArcInc = 0.005)
submitJob(2)
# ----------------------------------------------------------------------------------------------- #
...
def Extraction(jobNo):
    mdb.jobs['Job-'+str(jobNo)+'-NegModified'].waitForCompletion()
    odb = visualization.openOdb(path = 'Job-'+str(jobNo)+'-NegModified.odb')
    ...
    odb.close()
    mdb.jobs['Job-'+str(jobNo)+'-PosModified'].waitForCompletion()    
    odb = visualization.openOdb(path = 'Job-'+str(jobNo)+'-PosModified.odb')
...
jobs = ['Job-1-Neg', 'Job-1-NegModified', 'Job-1-Pos', 'Job-1-PosModified', 'Job-2-Neg', 
    'Job-2-NegModified', 'Job-2-Pos', 'Job-2-PosModified', 'Job-3-Neg', 'Job-3-NegModified', 
    'Job-3-Pos', 'Job-3-PosModified', 'Job-4-Neg', 'Job-4-NegModified','Job-4-Pos', 
    'Job-4-PosModified', 'Job-5-Neg', 'Job-5-NegModified', 'Job-5-Pos', 'Job-5-PosModified', 
    'Job-6-Neg', 'Job-6-NegModified', 'Job-6-Pos', 'Job-6-PosModified']
